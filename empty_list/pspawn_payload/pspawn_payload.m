...
#define PSPAWN_PAYLOAD_DYLIB "/var/containers/Bundle/dylibs/pspawn_payload.dylib"
#define AMFID_PAYLOAD_DYLIB "/var/containers/Bundle/dylibs/amfid_payload.dylib"
...
int fake_posix_spawn_common(pid_t * pid, const char* path, const posix_spawn_file_actions_t *file_actions, posix_spawnattr_t *attrp, char const* argv[], const char* envp[], pspawn_t old) {
    ...
    if (current_process == PROCESS_LAUNCHD) {
        if (strcmp(path, "/usr/libexec/xpcproxy") == 0) {
            ...
            if (startd != NULL) {
                const char **whitelist = xpcproxy_whitelist;
                
                while (*whitelist) {
                    if (strstr(startd, *whitelist)) {
                        DEBUGLOG("xpcproxy for '%s' which is in whitelist, injecting", startd);
